...
def load_state_dict_safely(model: nn.Module, ckpt_path: str) -> Tuple[List[str], List[str], Dict]:
    """
    返回：
      missing, unexpected, info
    info里包含：
      - remap_head: 是否做过 head/classifier/fc -> backbone.fc 的映射
      - critical_missing: 是否缺失关键分类头参数
    """
    try:
    ...
    # ---- 自动把不同命名的分类头映射到 backbone.fc ----
    info = {"remap_head": False, "critical_missing": False}

    model_keys = set(model.state_dict().keys())
    # 目标头参数名（你这个封装是 backbone.fc.*）
    tgt_w = "backbone.fc.weight" if "backbone.fc.weight" in model_keys else None
    tgt_b = "backbone.fc.bias"   if "backbone.fc.bias" in model_keys else None
    ...
    if tgt_w is not None:
        ...
        for sw, sb in src_pairs:
            if (sw in new_sd) and (tgt_w not in new_sd):
                # 可选：检查shape匹配再映射（更稳）
                if tuple(new_sd[sw].shape) == tuple(model.state_dict()[tgt_w].shape):
                    new_sd[tgt_w] = new_sd.pop(sw)
                    info["remap_head"] = True
            if (sb in new_sd) and (tgt_b not in new_sd):
                if tuple(new_sd[sb].shape) == tuple(model.state_dict()[tgt_b].shape):
                    new_sd[tgt_b] = new_sd.pop(sb)
                    info["remap_head"] = True
    # # 如果缺少分类头bias，就补0，避免初始化带来评测波动
    # if "backbone.fc.bias" in model.state_dict() and "backbone.fc.bias" not in new_sd:
    #     new_sd["backbone.fc.bias"] = torch.zeros_like(model.state_dict()["backbone.fc.bias"])
    #     print("[state_dict] backbone.fc.bias missing -> filled with zeros")
    # ---- PATCH: 如果缺少分类头 bias，则补 0，避免随机初始化影响评测 ----
    if "backbone.fc.bias" in model.state_dict() and "backbone.fc.bias" not in new_sd:
        new_sd["backbone.fc.bias"] = torch.zeros_like(model.state_dict()["backbone.fc.bias"])
        print("[state_dict] backbone.fc.bias missing -> filled with zeros")
    ...
    # 关键缺失判断：分类头没加载到就标记为 mismatch
    critical = {"backbone.fc.weight", "backbone.fc.bias"}
    if any(k in critical for k in missing):
        info["critical_missing"] = True
    ...
    if missing or unexpected:
        ...
        if info["remap_head"]:
            print("[state_dict] head->backbone.fc remapped")

    # return missing, unexpected, info
